...
$sql="INSERT INTO `project`(`project_sr_no`,`project_date`,  `project_so_no`,`project_challan_no`, `project_name`, `project_po_customer`,`project_po_amount`, `project_bill_date`) VALUES (
'$project_sr_no','$project_date','$project_so_no','$project_challan_no','$project_name','$project_po_customer','$project_po_amount','$project_bill_date')"; 
...
<script type="text/javascript">
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">  
				...
				<label>SPECIAL NOTE:</label>
					  ...
					  <option value="<?php echo $row["componenet_name_product"];?>"> <?php echo $row["componenet_name_product"]; ?></option>
							  ...
							  <Select name="project_item" class="form-control" id="project_item1">
								 ...
								 <Select name="project_item_raw_material" id="project_item_raw_material1" class="form-control" >
									...
									<option>--Please Select--</option>	
                                             ...
                                             <div class="form-group">
										     <label>AVAILABLE RAW QTY</label>
										      <input type="text" name="project_qty_available" id="project_qty_available1" 
										        value="" class="form-control"/>
												 </div>
											</div>
											...
											<div class="col-md-2">
                                             <div class="form-group">
										     <label>MATERIAL WEIGHT</label>
										      <input type="text" name="material_weight" id="material_weight1" 
										        value="" class="form-control"/>
												 </div>
											</div>
											...
											<div class="col-md-2">
                                             <div class="form-group">
										     <label>UNIT</label>
										      <input type="text" name="project_item_unit" id="project_item_unit1" 
										        value="" class="form-control"/>
												 </div>
											</div>
											<div class="col-md-2">
	...
	<script type="text/javascript">
        ...
        $("#project_item_raw_material1").change(function(){
	      var project_item_raw_material1=$("#project_item_raw_material1").val();
			
			$.ajax({
			type:'post',
			url: "project_raw_material_detail_fetch.php",
			data:'project_item_raw_material1='+project_item_raw_material1,
			success: function(result){
			
			var ret_arr=result.split("~_~");
						
				$("#project_qty_available1").val(ret_arr[0]);
				$("#project_item_unit1").val(ret_arr[2]);
 ...
 <script type="text/javascript">
  ...
  <script type="text/javascript">
    	$(document).ready(function(){
        $("#project_item1").change(function(){
	      var project_item1=$("#project_item1").val();
			
			$.ajax({
			type:'post',
			url: "project_material_detail_fetch.php",
			data:'project_item1='+project_item1,
			success: function(result){
			
			var ret_arr=result.split("~_~");
						
				//$("#project_item_raw_material").val(ret_arr[0]);
				$("#material_weight1").val(ret_arr[1]);
			...
			success: function(result){
			 ...
			 var select = $("#project_item_raw_material1"), options = 'Please Select';
             select.empty(); 
			  $('#project_item_raw_material1').append($('<option/>').attr("value",""));
                          for (var i = 0, len = data.length; i < len; i++) 
                                  {
                                        var name = data[i];
                                        $('#project_item_raw_material1').append("<option value='"+name+"'>"+name+"</option>");
...
<script type="text/javascript">
		...
		var project_challan_no=$("#project_challan_no").val();
			...
			var data = jQuery.parseJSON(result);
			 ...
			 var select = $("#project_item1"), options = 'Please Select';
             select.empty(); 
			  $('#project_item1').append($('<option/>').attr("value",""));
                                    for (var i = 0, len = data.length; i < len; i++) 
                                    {
                                        var name = data[i];
                                        $('#project_item1').append("<option value='"+name+"'>"+name+"</option>");
