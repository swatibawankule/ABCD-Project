          ...
          <div class="content">
			...
			<!--
				 ...
				 <?php	Require_once('connect.php');
					$sql=mysqli_query($conn,"select  distinct bill_to from sale_invoice");
					while($row=mysqli_fetch_array($sql))
						{
				?>
														  <option value="<?php echo $row["bill_to"];?>"> <?php echo $row["bill_to"]; ?></option>
															<?php
															}
															?>
													 </select>
...
-->
                <div class="row">
                    ...
                    <div class="col-md-12">
					 ...
					 <button style="float:right;margin-right:20px" type="submit" name="btn_new_party"  onclick="window.location.href='new_raw_material.php'" class="btn  btn-fill btn-success" style="float:right;"> New Raw Material</button>
                  
                        <div class="card">
                            <div class="header">
							   <h4 class="title">Raw Material Details</h4>
                             </div>
                            <div class="content">
                                <div class="container-fluid">
                                    <div class="row">
							  ...
							  <thead>
								  ...
								  <th align="center">Casting Weight</th>
...
$statement = "`raw_material` ORDER BY `sr_no` ASC"; // Change `records` according to your table name.
...
if (mysqli_num_rows($results) != 0) {
    ...
    while ($row = mysqli_fetch_array($results)) {
		echo "<tr>
                                      ...
                                      <td>$row[sr_no]</td>
                                      <td>$row[material]</td>
									  <td>$row[material_melting_loss]</td>
                                      <td>$row[material_casting_wt]</td>
                                      <td>$row[material_hsn_sac_code]</td>
   									<td>$row[unit]</td>
									  
										 
										  
                                      <td><a href=edit_sale_invoice.php?sr_no=$row[sr_no] class='btn btn-fill btn-primary'>Edit</a>
									
									 </a></td>
                                     <td><a href=view_sale_invoice.php?sr_no=$row[sr_no] class='btn btn-fill  btn-info'>View</a>
									</td>
								    <td><a style=\"width:70px;padding:5px\" onClick=\"javascript: return confirm('Are you sure you want to delete Sale Invoice details');\" class='btn btn-fill btn-warning' href='delete_sale_invoice.php?sr_no=$row[sr_no]'>Delete</a>
									 </td>
									 </a></td>
