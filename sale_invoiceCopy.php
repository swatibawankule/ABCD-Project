		   ...
		   <form method="post" id="form" align="center" >  
									  <div class="row">
										...
										<option>---Please Select---</option>
											...
											$sql=mysqli_query($conn,"select distinct project_name from project_item");
								 ...
								 <Select name="bill_to" class="form-control" id="bill_to">
													<option>---Select Bill To---</option>
														 <?php
															Require_once('connect.php');
															$sql=mysqli_query($conn,"select customer_name from customer");
															while($row=mysqli_fetch_array($sql))
															 {
															?>
   ...
   <a href=abcd_sale_invoice.php?invoice_no=<?php echo $acc_no-1 ; ?> class="btn btn-success btn-fill" style="float:right;margin-top:20px;margin-left:5px">Print</a>
	...
	<script type="text/javascript">
	 ...
	 <script type="text/javascript">
		...
		var dataString = 'invoice_no=' + invoice_no + '&project_no=' + project_no + '&sr_no=' + sr_no + '&product_name='  + product_name + '&unit=' + unit+ '&qty=' + qty + '&rate=' + rate + '&project_no=' + project_no + '&amount=' + amount;
    ...
    <script type="text/javascript">
        ...
        $("#bill_to").change(function(){
			...
			success: function(result){
				...
				$("#gst_no").val(ret_arr[4]);
				$("#contc_person").val(ret_arr[5]);
				$("#contc_no").val(ret_arr[6]);
				$("#c_no").val(ret_arr[7]);
