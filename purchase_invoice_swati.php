...
$po_sr_no=$_REQUEST['po_sr_no'];
	$query = "SELECT * from purchase_order where po_sr_no ='".$po_sr_no."'"; 
	$result = mysqli_query($conn, $query) or die ( mysqli_error());
	$row = mysqli_fetch_assoc($result);
	
 $po_sr_no=$row['po_sr_no'];
	    $bill_date=$row['bill_date'];
		
		$timestamp = strtotime($bill_date);
        $bill_date = date("d-m-Y", $timestamp);
		//$powo=$row['powo'];
		...
		$po_special_note=$row['po_special_note'];
...
<table border="1" style="width: 1000px; height: 150px; border-collapse: collapse;padding:0;margin:0" >
 ...
 <table border="1"  style="width: 1000px; height:100px; border-collapse: collapse;padding:0;margin:0" >
  <tr>
   ...
   <td align="center"><b>Description Of Services</b></td>
  <td align="center"><b>Due on</b></td>
