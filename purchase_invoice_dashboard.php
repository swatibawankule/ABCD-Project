	                     ...
	                     <div class="col-md-5">
							...
							<option>---Select Supplier Name---</option>
									 <?php		Require_once('connect.php');
															$sql=mysqli_query($conn,"select distinct po_supplier from  purchase_order");
															while($row=mysqli_fetch_array($sql))
															 {
															?>
								 <option value="<?php echo $row["po_supplier"];?>"> <?php echo $row["po_supplier"]; ?></option>
															<?php
															}
															?>
													 </select>
...
<div class="form-group">
	 ...
	 <button id="excel" class="btn  btn-fill btn-success" style="float:right;margin-top:25px">Export To Excel</button>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
					 <button style="float:right;margin-right:20px" type="submit" name="btn_new_party"  onclick="window.location.href='purchase_invoice.php'" class="btn  btn-fill btn-success" style="float:right;"> New Purchase Invoice</button>
                  
                        <div class="card">
                            <div class="header">
							   <h4 class="title">Purchase Invoice Details</h4>
                             </div>
                            <div class="content">
                                <div class="container-fluid">
                                    <div class="row">
...
$statement = "`purchase_order` ORDER BY `po_sr_no` ASC"; // Change `records` according to your table name.
...
if (mysqli_num_rows($results) != 0) {
    ...
    while ($row = mysqli_fetch_array($results)) {
		echo "<tr>
                                      ...
                                      <td><a href=edit_customer.php?po_sr_no=$row[po_sr_no] class='btn btn-fill btn-primary'>Edit</a>
									
									 </a></td>
                                     <td><a href=view_customer.php?po_sr_no=$row[po_sr_no] class='btn btn-fill  btn-info'>View</a>
									</td>
									 <td><a href=view_customer.php?po_sr_no=$row[po_sr_no] class='btn btn-fill  btn-info'>View</a>
									</td>
									
									
                                       <td><a style=\"width:70px;padding:5px\" onClick=\"javascript: return confirm('Are you sure you want to delete Customer details');\" class='btn btn-fill btn-warning' href='delete_customer.php?po_sr_no=$row[po_sr_no]'>Delete</a>
									 </td>
									 </a></td>
